...
Hcoeff   = -0.05  # =(Hd-H0)/D0, where H0 is the wse associated to D0 and Hd is the imposed downstream BC
Qcoeff   = 1  # =Qa/Q0
Qscoeff  = 1  # =Qsa/Qs0
...
Q0    = uniFlowQ(RF, W_a, S0, D0, d50, g, ks0, C0, eps_c)*Qcoeff
Qs0   = W_a*np.sqrt(g*delta*d50 ** 3)*phi00*Qscoeff
...
# Downstream BC
H0 = (eta_ab[0,-1]-S0*dx/2+D0)+D0*Hcoeff
...
for n in range(0, maxIter):
    ...
    deltaEta[n,:-1] = (eta_ab[n,:]-eta_ac[n,:])/D0
    deltaEta[n,-1]  = deltaEta[n,-2]

    # Downstream BC: H(t)=H0
    D_ab[-1] = H0-eta_ab[n,-1]+S_ab[-1]*dx/2
    D_ac[-1] = H0-eta_ac[n,-1]+S_ac[-1]*dx/2
