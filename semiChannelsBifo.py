...
# Model settings
bifoSwitch = 0 # if ==1, two anabranches are attached at the downstream end of the semichannels
st         = 1 # Solid transport switch: 0=fixed bed; 1=movable bed
bedIC      = 'shifted' # 'exp' adds a gradual inlet step with an exponential trend, so that deltaEta[-1 ]=inStep[0]; 'flat'; 'lastCell'; 'triangular'
...
# Space-time domain for the semichannels
...
for n in range(0, maxIter):
    ...
    # First solve the 1D profile in channel a, then compute Dab, Dac and Qy
    D_a         = buildProfile(RF,D_a[-1],Q0,W_a,0.5*(S_ab+S_ac),d50,dx,g,C0,eps_c)
    eta_ab_i    = (eta_ab[n,1:]+eta_ab[n,:-1])*0.5
    eta_ac_i    = (eta_ac[n,1:]+eta_ac[n,:-1])*0.5
    deltaEta    = eta_ab_i-eta_ac_i
    D_ab [1:-1] = D_a[1:-1]-0.5*deltaEta
    D_ac [1:-1] = D_a[1:-1]+0.5*deltaEta
    D_ab [0]    = D_a[0]-0.5*(eta_ab[n,0]+S_ab[0]*dx/2-eta_ac[n,0]-S_ac[0]*dx/2)
    D_ac [0]    = D_a[0]+0.5*(eta_ab[n,0]+S_ab[0]*dx/2-eta_ac[n,0]-S_ac[0]*dx/2)
    Q_ab [0]    = Q0/2
    Q_ac [0]    = Q0/2
    for i in range(0,nc):
    ...
    for i in range(nc-1,0,-1):
        ...
        for k in range(maxIterQy):
            ...
            Q_yMV_control .append(Q_yMV/(0.5*(Q_ab[i+1]-Q_ac[i+1])))
    ...
    # Shields, Qs and eta update for the semchannels
...
if st == 1:
    ...
    for i in range(numPlots):
        ...
        myPlot(nFig,   xc/W_a, (eta_a[plotTimeIndex,:]-eta0[:])/D0, ('t=%3.1f Tf' % (plotTimeIndex*dt/Tf)), color=bed_colors[i])
        myPlot(nFig+1, xc/W_a, (eta_ab[plotTimeIndex,:]-eta0)/D0, ('t=%3.1f Tf' % (plotTimeIndex*dt/Tf)), color=bed_colors[i])
        myPlot(nFig+2, xc/W_a, (eta_ac[plotTimeIndex,:]-eta0)/D0, ('t=%3.1f Tf' % (plotTimeIndex*dt/Tf)), color=bed_colors[i]) 
    nFig += 2
...
nFig += 1
myPlot(nFig,xi/W_a,D_a/D0,'Mean water depth','Cross-section-averaged water depth along the channel','x/W_a [-]','D_a/D0 [-]')
myPlot(nFig+1,xi/W_a,D_ab/D0,'ab water depth','ab depth','x/W_a [-]','D_ab/D0 [-]')
myPlot(nFig+2,xi/W_a,D_ac/D0,'ac water depth','ac depth','x/W_a [-]','D_ac/D0 [-]')
nFig += 2

# Plot water discharges in 2D
Qa_disp = np.vstack([Q_ac/Q0, Q_ab/Q0])
nFig += 1
plt.figure(nFig)
plt.imshow(Qa_disp, cmap='Blues', vmin=np.amin(Qa_disp), vmax=np.amax(Qa_disp), aspect='auto')
plt.xticks(range(0,nc,int(nc/10)), range(0,nc,int(nc/10)))
plt.yticks(range(2), ['ac', 'ab'])
plt.title('Water discharge scaled with Q0 [-]')
plt.colorbar()

# Plot Qy along the channel
nFig += 1
myPlot(nFig,xc/W_a,Q_y/dx/(Q0/W_a),'q_y/q_0','Transverse discharge per unit length','x/W_a [-]','q_y/q_0 [-]')
nFig_qy = nFig
